...
def tag_2_kraft_kpi_info(dataset,pulp_kpi_calc_logfile_path):
    ...
    try:
        ...
        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0252:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0254:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_2_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0256:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0207:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_4_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])
    ...
    try:
        ...
        # Monthly Consumptions
        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0226:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0228:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_2_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0230:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])
    ...
    try:
        ...
        # Monthly Consumptions
        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0243:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0245:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_2_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0247:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])
    ...
    try:
        ...
        # monthly consumptions
        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0234:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0236:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_2_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0238:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code']=="RPL.111XC0240:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx],'process_line_4_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx],'kpi_value'])
    ...
    try:
        ...
        # Monthy Consumptions
        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0261:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0263:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_2_m'] = list( dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0265:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_3_m'] = list( dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])
    ...
    try:
        ...
        print('MP Steam Consumption found ')
    except:
    ...
    try:
        ...
        # Monthly Consumptions
        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0317:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0319:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_2_m'] = list( dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0321:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0323:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_4_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        print('Total Steam Consumption FOUND ')
    except:
    ...
    try:
        ...
        # Monthly Consumptions
        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0271:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_1_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0273:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_2_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])

        kpi_fetch_idx = np.where(dataset['tag_code'] == "RPL.111XC0275:afc".lower())
        dataset.loc[dataset.index[kpi_main_idx], 'process_line_3_m'] = list(dataset.loc[dataset.index[kpi_fetch_idx], 'kpi_value'])
    ...
    try:
        ...
        error_message = "Woodchip Consumption - Consumption tag RPL.111XC0211:afc FOUND "
        print(error_message)
        
    except:
        error_message = "Woodchip Consumption - Consumption tag RPL.111XC0211:afc cannot be found"
        print(error_message)
        logging.log_message(pulp_kpi_calc_logfile_path, error_message)
...
def kraft_kpi_calcs(raw_data, kpi_info_columns):
    ...
    kraft_pulp_kpi['process_line_1_m'] = np.nan
    kraft_pulp_kpi['process_line_2_m'] = np.nan
    kraft_pulp_kpi['process_line_3_m'] = np.nan
    kraft_pulp_kpi['process_line_4_m'] = np.nan
