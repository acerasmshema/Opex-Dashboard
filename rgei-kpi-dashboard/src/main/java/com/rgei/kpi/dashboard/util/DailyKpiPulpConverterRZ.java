...
public class DailyKpiPulpConverterRZ {
	...
	public static ProcessLineResponse prePareResponse(List<ProcessLineEntity> processLineEntites,
		...
		logger.info("Prepare process line response for RZ");
	...
	private static ProcessLineResponse populateTarget(List<ProcessLineEntity> processLineEntites,ProcessLineResponse response,MillBuKpiCategoryEntity millBuKpiCategoryEntity) {
	...
	public static ProcessLineAnnualResponse prePareResponse(MillBuKpiCategoryEntity millBuKpiCategoryEntity,
		...
		logger.info("Prepare process line annual response for RZ");
	...
	public static List<ProcessLineSeries> prePareDailyTargetResponse(List<DailyKpiPulpEntity> dailyKpiPulpEntities) {
		logger.info("Prepare daily target response for RZ");
	...
	public static List<DateRangeResponse> createDailyKpiPulpResponseForBarChart(List<DailyKpiPulpEntity> dailyKpiPulpEntities) {
		logger.info("Create daily Kpi Pulp response for Bar Chart - RZ");
		...
		dailyKpiPulpEntities.forEach(item -> {
			...
			seriesObjects.add(new SeriesObject(Utility.dateToStringConvertor(item.getDatetime(), DashboardConstant.DATE_FORMAT), 
					Utility.parseProcessLineValue(BigDecimal.valueOf(item.getProcessLine4()).setScale(0, RoundingMode.CEILING).doubleValue())));
			seriesObjects2.add(new SeriesObject(Utility.dateToStringConvertor(item.getDatetime(), DashboardConstant.DATE_FORMAT), 
					Utility.parseProcessLineValue(BigDecimal.valueOf(item.getProcessLine5()).setScale(0, RoundingMode.CEILING).doubleValue())));
		});
		return resultList;
	}

	public static List<DateRangeResponse> createDailyKpiPulpResponseForAreaChart(List<DailyKpiPulpEntity> dailyKpiPulpEntities) {
		logger.info("Create daily Kpi Pulp response for Area Chart - RZ");
		List<DateRangeResponse> resultList = new ArrayList<>();
		List<String> lineList = getRequestedProcessLines();
		...
		dailyKpiPulpEntities.forEach(item -> {
			...
			Collections.addAll(seriesList,
					new SeriesObject(lineList.get(0), Utility.parseProcessLineValue(BigDecimal.valueOf(item.getProcessLine4()).setScale(0, RoundingMode.CEILING).doubleValue())),
					new SeriesObject(lineList.get(1), Utility.parseProcessLineValue(BigDecimal.valueOf(item.getProcessLine5()).setScale(0, RoundingMode.CEILING).doubleValue())));
