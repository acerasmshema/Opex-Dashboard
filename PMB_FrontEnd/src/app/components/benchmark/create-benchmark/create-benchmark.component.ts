...
@Component({
  selector: 'app-create-benchmark',
  templateUrl: './create-benchmark.component.html',
  styleUrls: ['./create-benchmark.component.scss'],
  providers: [MessageService]
})
export class CreateBenchmarkComponent implements OnInit {

  createBenchmarkForm: FormGroup;
  locations: Location[];
  ...
  constructor(private fb: FormBuilder, private benchmarkService: BenchmarkService, private masterDataService: MasterDataService, private messageService: MessageService) {
    this.createBenchmarkForm = fb.group(
      {
        setName: "",
        setDescription: "",
        setLocation: "",
  ...
  public saveSet(data: any) {
    ...
    this.benchmarkService.saveSet(data).subscribe((data: any) => {
      if(data==null){
        this.showMessage('error', 'unsuccessful :', 'Benchmark could not be created');
      }else{
        this.showMessage('success', 'Successful :', 'Benchmark created Successfully');
        this.createBenchmarkForm = this.fb.group(
          {
            setName: "",
            setDescription: "",
            setLocation: "",
